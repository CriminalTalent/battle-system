...
// - 안전한 널가드와 렌더링 방어, 경미한 폴백 유틸 포함(디자인 영향 없음)

(function () {
  'use strict';

  // ------------------------------------------------------------
  // 안전 폴백 유틸 (프로젝트 공용 유틸이 없는 경우 대비)
  // ------------------------------------------------------------
  const UI = window.UI || {
    ...
    validateForm: (form, schema) => {
      const data = {};
      const errors = [];
      if (!form) return { valid: false, data, errors: ['폼이 없습니다'] };
      Object.entries(schema || {}).forEach(([id, rule]) => {
        const el = form.querySelector('#' + id);
        const val = (el && el.value || '').trim();
        data[id] = val;
        if (rule.required && !val) errors.push(`${rule.label || id}을(를) 입력하세요`);
      });
    ...
    calculateHpPercent: (hp, maxHp) => {
      const mx = Math.max(1, Number(maxHp || 100));
      const cur = Math.max(0, Math.min(mx, Number(hp || mx)));
      return (cur / mx) * 100;
    },
    scrollToBottom: (el) => { if (el) el.scrollTop = el.scrollHeight; }
  };

  // PyxisSocket 폴백 훅 (실 서버에서는 프로젝트 제공 객체 사용)
  const PyxisSocket = window.PyxisSocket || {
    socket: null,
    _listeners: {},
    init() { this.socket = { connected: true }; this._emit('connection:success'); },
    ...
    authenticate: async () => { setTimeout(() => PyxisSocket._emit('authSuccess', { state: {} }), 50); },
    sendPlayerAction: async () => {},
    sendChat: () => {},
    cleanup() { this._listeners = {}; }
  };

  // PyxisFX 최소 폴백 (시각효과 유틸 없음 대비)
  const PyxisFX = window.PyxisFX || {
    ...
    stopTurnTimer() {},
    attachTurnTimer() {},
    vibrate: (p) => { if (navigator.vibrate) try { navigator.vibrate(p); } catch (_) {} },
    showHit: () => {},
    showDodge: () => {},
    showHeal: () => {},
    sparkAt: () => {}
  ...
  // 대상 선택기 폴백
  window.PyxisTarget = window.PyxisTarget || {
    _shown: false,
    isShown() { return this._shown; },
    show(title, list, cb) {
      this._shown = true;
      const name = prompt(`${title}\n` + list.map(p => `${p.name} (${p.id})`).join('\n') + '\n이름 또는 ID를 입력:');
      const picked = list.find(p => p.name === name || String(p.id) === name);
      if (picked) cb(picked);
      this._shown = false;
    }
  };
  ...
  class PyxisPlayer {
    ...
    init() {
      ...
      if (PyxisFX.enhanceClicks) PyxisFX.enhanceClicks(document);
    }
    ...
    setupEventListeners() {
      ...
      // 액션 버튼
      this.btnAttack && this.btnAttack.addEventListener('click', () => {
        this.addButtonPressEffect(this.btnAttack, 'attack');
        this.doAttack();
      });
      this.btnDefend && this.btnDefend.addEventListener('click', () => {
        this.addButtonPressEffect(this.btnDefend, 'defend');
        this.sendAction('defend');
      });
      this.btnDodge && this.btnDodge.addEventListener('click', () => {
        this.addButtonPressEffect(this.btnDodge, 'dodge');
        this.sendAction('evade');
      });
      this.btnUseItem && this.btnUseItem.addEventListener('click', () => {
        this.addButtonPressEffect(this.btnUseItem, 'item');
        this.doUseItem();
      });
      this.btnPass && this.btnPass.addEventListener('click', () => {
        this.addButtonPressEffect(this.btnPass, 'pass');
        this.confirmPass();
      });

      // 채팅
      this.chatSendBtn && this.chatSendBtn.addEventListener('click', () => this.sendChat());
      this.chatInput && this.chatInput.addEventListener('keypress', (e) => {
      ...
      document.addEventListener('visibilitychange', () => {
        if (!document.hidden && this.joined && this.currentBattleId) {
          PyxisSocket.socket && PyxisSocket.socket.emit && PyxisSocket.socket.emit('requestState', { battleId: this.currentBattleId });
        }
      });
    ...
    setupSocketEvents() {
      const on = (ev, fn) => PyxisSocket.on && PyxisSocket.on(ev, fn);

      on('connection:success', () => {
        this.connectionDot && this.connectionDot.classList.add('active');
        if (this.connectionText) this.connectionText.textContent = '연결됨';
        this.addConnectionPulse(true);
      });

      on('connection:disconnect', () => {
        this.connectionDot && this.connectionDot.classList.remove('active');
        if (this.connectionText) this.connectionText.textContent = '연결 끊김';
        this.addConnectionPulse(false);
      });

      // 인증
      on('authSuccess', (data) => this.handleAuthSuccess(data));
      on('authError', (msg) => {
        UI.error(`인증 실패: ${msg}`);
        this.addErrorPulse();
      });
      ...
      on('state', (s) => this.handleStateUpdate(s));
      on('state:update', (s) => this.handleStateUpdate(s));
      on('battleUpdate', (s) => this.handleStateUpdate(s)); // 일부 서버 구현 호환
      ...
      on('phase:change', (phase) => {
        ...
        else PyxisFX.stopTurnTimer && PyxisFX.stopTurnTimer();
      });

      // 액션 결과
      on('action:success', (result) => this.handleActionFX(result));
      on('actionSuccess', (result) => this.handleActionFX(result));
    ...
    initFromUrl() {
      ...
      if (b && t && n) {
        ...
        PyxisSocket.on && PyxisSocket.on('connection:success', () => setTimeout(() => this.authenticate(), 300));
      } else {
    ...
    async authenticate() {
      ...
      try {
        ...
        await (PyxisSocket.authenticate && PyxisSocket.authenticate({
          role: 'player',
          battleId: v.data.authBattleId,
          token: v.data.authToken,
          name: v.data.authName,
          playerId: v.data.authName, // 서버가 name 기반 식별 허용 시 호환
    ...
    handleAuthSuccess(data) {
      ...
      if (!this.battleTimerStarted) {
        PyxisFX.startBattleTimer && PyxisFX.startBattleTimer(60 * 60 * 1000);
    ...
    normalizeState(raw) {
      if (!raw) return { players: {}, turn: { pending: [] }, status: 'wait' };
    ...
    normalizePlayer(p, id) {
      ...
      const items = {
        // 디터니
        dittany: itemsRaw.dittany ?? itemsRaw.Dittany ?? itemsRaw.diterni ?? itemsRaw.ditany ?? itemsRaw['디터니'] ?? 0,
        // 공격/방어 보정기
        atkBoost: itemsRaw.atkBoost ?? itemsRaw.attackBoost ?? itemsRaw['attack_boost'] ?? itemsRaw['공격 보정기'] ?? 0,
        defBoost: itemsRaw.defBoost ?? itemsRaw.defenseBoost ?? itemsRaw['defense_boost'] ?? itemsRaw['방어 보정기'] ?? 0
      };
    ...
    normalizeTeamName(teamVal) {
      ...
      if (['a', 'team1', 'phoenix', 'phoenixes', '불사조', '불사조 기사단'].includes(t)) return 'A';
      if (['b', 'team2', 'death', 'death eaters', 'deatheaters', '죽음', '죽음을 먹는 자들'].includes(t)) return 'B';
    ...
    startMyTurnTimer() {
      PyxisFX.attachTurnTimer && PyxisFX.attachTurnTimer(this.actionArea, () => this.autoPass());
      PyxisFX.startTurnTimer && PyxisFX.startTurnTimer(5 * 60 * 1000);
      PyxisFX.vibrate && PyxisFX.vibrate([40, 40, 40]);
    ...
    async sendAction(type, extra = {}) {
      if (!PyxisSocket.isConnected || !PyxisSocket.isConnected()) {
        UI.error('서버에 연결되어 있지 않습니다');
        return;
      }
      try {
        await (PyxisSocket.sendPlayerAction && PyxisSocket.sendPlayerAction(
          type,
    ...
    handleActionFX(result) {
      ...
      if (result.type === 'attack') {
        if (result.dodge) {
          PyxisFX.showDodge && PyxisFX.showDodge(targetEl);
          this.addCombatText(targetEl, '회피', 'var(--text-dim)');
        } else {
          PyxisFX.showHit && PyxisFX.showHit(targetEl, result.damage || 0, { crit: !!result.crit, blocked: !!result.block });
        ...
        if (result.item === '디터니') {
          PyxisFX.showHeal && PyxisFX.showHeal(targetEl, result.healed || 10);
          this.addCombatText(targetEl, `+${result.healed || 10} HP`, 'var(--success)');
        } else {
          PyxisFX.sparkAt && PyxisFX.sparkAt(targetEl, { ring: true });
    ...
    sendChat() {
      ...
      PyxisSocket.sendChat && PyxisSocket.sendChat({
  ...
  document.addEventListener('DOMContentLoaded', () => {
    try {
      ...
      console.error('[PYXIS Player] init error:', e);
    }
  });
})();
