...
(() => {
  ...
  function setLinks() {
    ...
    const play  = `${base}/player${q}`;   // ← 여기만 /play → /player 로 수정
  ...
  function renderState(state){
    // 기존 UI 구조 가정: 팀/플레이어/로그 등 갱신
    // 이 파일에서는 디자인/레이아웃 불변 원칙에 따라 로직만 유지
    // 필요한 DOM 갱신 코드가 기존에 있으면 그대로 동작
  }
  ...
  async function startBattle() {
    ...
    try {
      await jfetch(API.battleStart(id), { method: 'POST' });
      $('#battleStatus') && ($('#battleStatus').textContent = '진행중');
      UI.log('배틀 시작','ok');
    } catch (e) {
      UI.log('배틀 시작 실패: ' + e.message, 'err');
    }
  }
  ...
  async function pauseBattle() {
    ...
    if (!id) return UI.log('배틀 ID가 비어있습니다','err');
    try {
      await jfetch(API.battlePause(id), { method: 'POST' });
      UI.log('일시정지','ok');
    } catch (e) {
      UI.log('일시정지 실패: ' + e.message, 'err');
    }
  }
  ...
  async function resumeBattle() {
    ...
    if (!id) return UI.log('배틀 ID가 비어있습니다','err');
    try {
      await jfetch(API.battleResume(id), { method: 'POST' });
      UI.log('재개','ok');
    } catch (e) {
      UI.log('재개 실패: ' + e.message, 'err');
    }
  }
  ...
  async function endBattle() {
    ...
    try {
      ...
      $('#battleStatus') && ($('#battleStatus').textContent = '종료');
      UI.log('종료','ok');
    } catch (e) {
      UI.log('종료 실패: ' + e.message, 'err');
    }
  }
