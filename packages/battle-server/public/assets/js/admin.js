/* PYXIS Admin – 인터페이스 보완판 (표기: '전투 참여자') */
(function(){
  'use strict';
  ...
  const playerUrlEl = $('#playerUrl');      // id는 유지 (호환)
  ...
  const btnAddPlayer = $('#btnAddPlayer');  // id는 유지
  ...
  async function createBattle(){
    ...
    try{
      ...
      if(res.ok){ const data = await res.json(); currentBattleId = data.id || data.battleId || null; }
      else{ throw new Error('REST create failed'); }
    }catch(e){
    ...
    refreshAllLinks();
    if(socket && socket.connected && currentBattleId){ socket.emit('admin:join',{battleId:currentBattleId}); }
  ...
  async function generateOtp(role){
    ...
    try{
      ...
      if(res.ok){ const data = await res.json(); if(data && data.otp) return String(data.otp); }
      else{ throw new Error('OTP api not ok'); }
    }catch(e){ return genLocalOtp(); }
    return genLocalOtp();
  }
  ...
  async function addPlayer(){
    ...
    if(!ok && socket && socket.connected){
      socket.emit('admin:addPlayer',{battleId:currentBattleId,player},(resp)=>{
        ...
        else{ addPlayerMsg.textContent='추가 실패'; setTimeout(()=>addPlayerMsg.textContent='',1500); }
      });
    }else if(ok){
      finalize();
    }else{
    ...
    function finalize(){
      ...
      addRosterItem({ id:pid, name:player.name, team:player.team, hp:player.hp, stats:player.stats, items:player.items });
  ...
  function ctl(kind){
    ...
    fetch(`/api/battles/${encodeURIComponent(currentBattleId)}/${kind}`,{method:'POST'})
      .then(res=>{
        ...
        else if(socket && socket.connected){ socket.emit('admin:control',{battleId:currentBattleId,action:kind}); toastMsg('처리되었습니다'); }
      })
      .catch(()=>{
        if(socket && socket.connected){ socket.emit('admin:control',{battleId:currentBattleId,action:kind}); toastMsg('처리되었습니다'); }
      });
  }
  ...
  btnGenPlayerOtp.addEventListener('click', async()=>{
    ...
    playerOtpEl.value = playerOtp; refreshAllLinks();
    toastMsg('전투 참여자 비밀번호가 발급되었습니다');
  });
  btnGenSpectatorOtp.addEventListener('click', async()=>{
    spectatorOtp = await generateOtp('spectator');
    spectatorOtpEl.value = spectatorOtp; refreshAllLinks();
    toastMsg('관전자 비밀번호가 발급되었습니다');
  });

  $$('.btn-copy').forEach(btn=>{ btn.addEventListener('click', ()=>{ const sel = btn.getAttribute('data-copy'); copyBySelector(sel); }); });
