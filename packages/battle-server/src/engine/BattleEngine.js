...
// 완전 교체용 - 정확한 로그 포맷 구현(오탈자/중복/문법 점검만, 로직/룰 흐름 변경 없음)
...
export function createBattleStore() {
  ...
  function start(id) {
    ...
    const rA = d10();
    const rB = d10();
    const totalA = sumA + rA;
    const totalB = sumB + rB;

    let first = 'A';
    if (totalB > totalA) first = 'B';
    if (totalA === totalB) first = (d10() >= d10() ? 'A' : 'B');
