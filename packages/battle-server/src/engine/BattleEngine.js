/* packages/battle-server/src/engine/BattleEngine.js  (ESM Drop-in)
 * - 선택 페이즈: '의도 로그'만 송출
 ...
 * - 🧩 안정화: 선택페이즈 타임아웃은 phaseToken 검증, 남은시간 기반 재확인
 * - 🧾 로그: 방어/회피는 “발동/시도” 로그를 먼저 찍고 → 공격 결과 로그가 뒤따르도록 정렬
 */
...
export function createBattleStore(){
  ...
  function startSelectPhase(b, team){
    ...
    // ✅ 남은시간 기반 타임아웃(phaseToken/phase 검증, 조기 트리거 방지)
    const myToken = b.phaseToken;
    const checkTimeout = () => {
      if(b.status!=='active') return;
      if(b.phaseToken!==myToken) return;                 // 다른 페이즈로 변경됨
      if(!(b.phase==='A_select'||b.phase==='B_select')) return;
      if(b.currentTeam!==team) return;

      const remain = (b.phaseEndsAt||0) - now();
      if(remain > 250) {
        // 아직 시간 남음 → 남은 시간만큼 다시 대기
        setTimeout(checkTimeout, Math.min(remain+10, 60_000)); // 최장 60초 단위로 재확인
        return;
      }
    ...
    const finalize = ()=>{
      ...
      if(!bothAlive(b)){
        ...
        if(win){
          ...
          b.status='ended'; b.phase='idle'; b.turnCursor=null; b.phaseEndsAt=null; touch(b); return;
        }
      }
  ...
  // 단일 공격 처리(방어/회피 로그를 먼저 보여주도록 정렬)
  function resolveSingleAttack(b, actor, target, useAtkBoost, defBoostIntended, allIntents, once){
    ...
    if(hasDodgeIntent && !once.dodgeTried.has(target.id)){
      ...
      pushLog(b, `→ ${target.name} 회피 판정 시도`, 'result'); // ✅ 시도 로그 먼저
    ...
    if(canDefend && !once.defendTried.has(target.id)){
      ...
      // ✅ 방어 발동 로그를 먼저
      if(useDefBoost){
        pushLog(b, `→ ${target.name} 방어 발동(보정기 ×2)`, 'result');
      }else{
