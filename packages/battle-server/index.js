// PYXIS Battle Server - Entry (with 5min Turn Timer)
// Node.js ≥ 18
...
battle = {
  ...
  adminToken,
  players: [{ id, name, team:"phoenix"|"eaters", stats:{atk,def,agi,luk}, items:[], avatar, hp, maxHp, ready }],
  turn: { current, lastChange },
  log: [{ t, type:"system"|"chat"|"cheer", message }],
  // 타이머
  turnTimer?: { endAt:number, iv?:NodeJS.Timeout }
}
*/
...
function startTurnTimer(battle) {
  ...
  battle.turnTimer.iv = setInterval(() => {
    ...
    if (remaining <= 0) {
      ...
      advanceTurn(battle); // 다음 턴 + 타이머 리셋
    }
  }, 1000);
}

function stopTurnTimer(battle) {
  if (battle?.turnTimer?.iv) {
    clearInterval(battle.turnTimer.iv);
  }
...
function advanceTurn(battle) {
  nextTurn(battle);
  // nextTurn은 turn.current/lastChange 갱신만 담당하므로 여기서 방송/타이머 재시작
