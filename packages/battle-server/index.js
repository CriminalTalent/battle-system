...
io.on('connection', (socket) => {
  ...
  socket.on('adminAuth', ({ battleId, token }) => {
    console.log(`[SOCKET] adminAuth 이벤트: battleId=${battleId}, token=${token}`);
    if (!battleId || !token) {
  ...
  socket.on('createBattle', ({ mode }) => {
    ...
    try {
      ...
      console.log(`[SOCKET] battleCreated 응답 전송:`, response);
      socket.emit('battleCreated', response);
      
      console.log(`[SOCKET] battle:update 전송`);
  ...
  socket.on('playerAuth', ({ battleId, name, token }) => {
    ...
    socket.emit('battle:update', serializeBattle(b));
    console.log(`[SOCKET] 전투 참가자 인증: ${name} -> ${battleId}`);
  });
  ...
  socket.on('player:action', ({ battleId, playerId, action }) => {
    try {
      ...
      if (player && battle.status === 'active') {
        ...
        console.log(`[SOCKET] 전투 참가자 액션: ${player.name} -> ${actionText}`);
      }
    } catch (error) {
