...
class BattleEngine {
  ...
  createBattle(mode = "1v1", adminId = null) {
    ...
    const configs = { "1v1": { playersPerTeam: 1 }, "2v2": { playersPerTeam: 2 }, "3v3": { playersPerTeam: 3 }, "4v4": { playersPerTeam: 4 } };
    const config = configs[mode] || configs["1v1"];

    const battle = {
      id, mode, status: "waiting", createdAt: Date.now(), adminId,
      teams: { team1: { name: "불사조 기사단", players: [] }, team2: { name: "죽음을 먹는자들", players: [] } },
      currentTeam: null, roundNumber: 1, turnNumber: 1, turnStartTime: null,
      battleLog: [], chatLog: [], config, winner: null, endReason: null,
      otps: { admin: this.generateOTP(), player: this.generateOTP(), spectator: this.generateOTP() },
    };
  ...
  addPlayer(battleId, { name, team, stats, inventory = [], imageUrl = "" }) {
    ...
    const player = {
      ...
      name, team,
      stats: { attack: clamp(stats?.attack), defense: clamp(stats?.defense), agility: clamp(stats?.agility), luck: clamp(stats?.luck) },
      inventory, imageUrl,
      hp: 100, maxHp: 100, alive: true, hasActed: false, isReady: false, isDefending: false, isDodging: false,
      buffs: {},
    };
  ...
  startBattle(battleId) {
    ...
    b.status = "ongoing"; b.turnStartTime = Date.now();
  ...
  endBattle(battleId, winner = null, reason = "") {
    const b = this.getBattle(battleId);
    if (!b) throw new Error("전투 없음");
    b.status = "ended"; b.winner = winner; b.endReason = reason || "전투 종료"; b.endedAt = Date.now();
    if (winner) this.addBattleLog(battleId, "system", `${b.teams[winner]?.name || winner} 팀 승리!`);
    else this.addBattleLog(battleId, "system", reason || "무승부");
    return b;
  }
  ...
  determineFirstTeam(b) {
    ...
    if (t1 > t2) { b.currentTeam = "team1"; this.addBattleLog(b.id,"system",`${b.teams.team1.name} 선공`); }
    else if (t2 > t1) { b.currentTeam = "team2"; this.addBattleLog(b.id,"system",`${b.teams.team2.name} 선공`); }
    else { this.addBattleLog(b.id,"system","동점! 재굴림"); this.determineFirstTeam(b); }
  }
  ...
  attack(b, attacker, targetId){
    const t = this.findPlayer(b, targetId); if(!t||!t.alive) throw new Error("대상 없음");
    let dmg = attacker.stats.attack + this.rollDice(20);
    if(attacker.buffs.attack_buff) dmg = Math.floor(dmg * 1.5);
    if (t.isDefending) {
      let def = t.stats.defense; if (t.buffs.defense_buff) def = Math.floor(def*1.5);
      dmg = Math.max(1, dmg - def);
    }
    const crit = this.rollDice(20) >= 20 - Math.floor(attacker.stats.luck/2);
    if (crit) dmg *= 2;
    t.hp = Math.max(0, t.hp - dmg); if (t.hp===0) t.alive = false;
  ...
  useItem(b, p, itemType, targetId){
    const idx = p.inventory.indexOf(itemType); if (idx === -1) throw new Error("아이템 없음");
    p.inventory.splice(idx,1);
    if (itemType === "디터니") {
      const t = targetId ? this.findPlayer(b, targetId) : p; if(!t) throw new Error("대상 없음");
      t.hp = Math.min(t.maxHp, t.hp + 10);
      this.addBattleLog(b.id,"action",`${p.name} → ${t.name} HP +10`); return;
    }
  ...
  addBattleLog(battleId, type, message){
    const b = this.getBattle(battleId); if(!b) return;
...
io.on("connection", (socket)=>{
  ...
  socket.on("adminAuth", ({ battleId, otp })=>{
    ...
    socket.join(battleId); socket.battleId = battleId; socket.role = "admin";
  ...
  socket.on("playerAuth", ({ battleId, playerId, otp, playerOTP })=>{
    ...
    if(!r.success) return socket.emit("authError", r.message);
    socket.join(battleId); socket.battleId = battleId; socket.role = "player"; socket.playerId = playerId;
  ...
  socket.on("spectatorAuth", ({ battleId, otp, spectatorName })=>{
    ...
    if(!r.success) return socket.emit("authError", r.message);
    socket.join(battleId); socket.battleId = battleId; socket.role = "spectator"; socket.spectatorName = spectatorName || "관전자";
  ...
  socket.on("chatMessage", ({ message })=>{
    ...
    let sender = "알 수 없음", senderType = socket.role || "system";
