...
app.post("/api/upload/avatar", upload.fields([
  { name:"avatar", maxCount:1 }, { name:"image", maxCount:1 }, { name:"file", maxCount:1 }
]), (req,res) => {
  try { uploadHandler(req,res); }
  catch(e){ res.status(400).json({ ok:false, error:String(e?.message||e) }); }
});

// 호환: POST /api/battles/:id/avatar -> 동일 동작
app.post("/api/battles/:id/avatar", upload.fields([
  { name:"avatar", maxCount:1 }, { name:"image", maxCount:1 }, { name:"file", maxCount:1 }
]), (req,res) => {
  try { uploadHandler(req,res); }
  catch(e){ res.status(400).json({ ok:false, error:String(e?.message||e) }); }
...
// SPA-like fallback only for other HTML GETs
...
app.post("/api/admin/battles/:id/links", (req, res) => {
  try {
    ...
    otpStore.set(`spectator_${battleId}`, {
      otp: spectatorOtp, role: "spectator", battleId,
    ...
    (battle.players || []).forEach((p, idx) => {
      ...
      otpStore.set(`player_${battleId}_${p.id}`, {
        otp, role:"player", battleId, playerId: p.id, playerName: p.name, team: p.team,
        expires: Date.now() + 2*60*60*1000,
      });
      const url = `${base}/player.html?battle=${battleId}&token=${otp}&playerId=${p.id}&name=${encodeURIComponent(p.name)}&team=${p.team}`;
...
// 호환: /api/battles/:id/links -> 위 라우트로 연결
app.post("/api/battles/:id/links", (req, res, next) => {
...
io.on("connection", (socket) => {
  ...
  socket.on("removePlayer", (payload, cb) => {
    console.log("[SOCKET] removePlayer(alias) <-", payload);
    const id = payload.battleId || socket.battleId;
    const b = battles.get(id);
    if (!b) return cb && cb({ error:"not_found" });
    const before = b.players.length;
    b.players = b.players.filter((x) => x.id !== payload.playerId);
    const removed = before !== b.players.length;
    if (removed) {
      pushLog(id, "admin", `참가자 삭제: ${payload.playerId}`);
      emitBattleUpdate(id);
    }
    cb && cb({ ok: removed });
  });
  ...
  socket.on("playerAuth", ({ battleId, password, token, otp, playerName }, cb) => {
    ...
    try {
      ...
      socket.join(id); socket.battleId = id; socket.playerId = player.id; socket.role = "player";
      const result = { ok:true, playerId:player.id, playerData:player, battle:b, message:"인증 성공! 전투에 참가했습니다.", success:true };
